...
textureLoader2.load("textures/depthmap.png", (loadedDepthTexture) => {
  const depthTexture = loadedDepthTexture;
  const originalTexture = textureLoader2.load("textures/original_image.png");

  // THREE.ImageLoader を使用してデプスマップ画像を読み込む
  const imageLoader = new THREE.ImageLoader();
  imageLoader.load("textures/depthmap.png", (loadedImage) => {
    // ImageData を取得するために Canvas を使用
 ...
 // デプスマップ画像を使用して頂点の高さを調整
  ...
  for (let i = 0; i < verticesCount; i++) {
    ...
    const depthValue = imageData.data[i * 4] / 255;
